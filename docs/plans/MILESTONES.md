...
> **⚠️ M10 is excepted from the rule above — its Status column is provenance, not state.** Read the
> board for M10, per the rescope note on that milestone. Nothing else in this section used to point
> forward to that note, so a reader working top-down reached M10 with no warning and computed a
> column that had deliberately stopped being maintained.
>
> **Everywhere else the rule holds, but a blank status is still a claim about this file rather than
> about the code.** The table goes stale in exactly the case where the work went *well*: a row is
> delivered under an adjacent piece of work that never comes back to tick it. An audit of every
> blank row found seven built-but-blank, and **all seven were in M10** — the milestone whose rows
> were knowingly left behind while differently-numbered tasks carried their text forward. M8 and M9
> produced none. So the rule is unsafe for one milestone and sound for the rest.
>
> **Read the source for the row you are about to pick up and confirm it is genuinely absent.** Three
> crews were dispatched onto already-built rows in a single session before a check caught it, and a
> crew that rebuilds working code can leave it worse than it found it. Note also that a bracketed
> number in a commit subject may be a pull-request number rather than a row in this file — the two
> numbering spaces collide, so searching commit subjects for a row number is not evidence.
...
| **111** | **P1 — Tool documentation on demand, and defaults that carry.** Two answers to the same problem, chosen so neither inflates the context it is trying to protect. **On demand — shipped.** The advertised `inputSchema` is already complete and correct — verified over a live `tools/list` — so what is missing is not the schema but the rules JSON Schema *cannot express*: `create_item`'s `originType: "person"` → `originPersonId` refinement, and `complete_item`'s entire conditional matrix (`shipped` 1–5, `how_verified` required only when `user_facing` is false), both enforced in runtime validators and invisible to a client. A `describe_tool` call (`src/lib/service/operations/describe-tool.ts`) returning the full contract for one named tool keeps that out of every turn's context, which is what a fatter description would cost (`PLAN.md`: every description sits in the agent's context on every turn). **Defaults that carry — shipped.** A session declares once at `POST /sessions/{id}/register` — autonomous or person-driven, and which person — and subsequent calls inherit it rather than restating it. Both halves are stored on the `Session` row (`personId`, `driveMode`) and resolved on every creation path by `src/lib/service/items/session-defaults.ts`, threaded through `insertItem` so all four creates inherit identically by construction rather than by four operations continuing to agree. Four rules bound it, and the last two carry the weight: an explicit value always wins; a declared person implies a person origin; the reverse inference is **never** made, because defaulting `originType` to `auto` for a session with no declared person would silently relabel work created on somebody's behalf as machine-originated; and an inherited person is validated against the database exactly as a typed one is, so a declaration can never write a dangling reference. A re-registration that does not restate the declaration refreshes rather than retracts it — the ordinary reconnect sends neither field, and clearing them there would un-declare a session mid-run. The summary guard's refusal now names `what_to_test` and `how_verified` outright instead of describing the conditional as a branch: `branch` is a real column on the item being completed, so that word cannot be used neutrally in this one refusal — it was read as a request for that column, sending callers to supply the wrong field | 30, 43 |`done` |
...
| **87** | Budget-window editor — per-window cards, the three boundary kinds in plain words, the band chart, drawn validation errors, the time scrubber, and presets. Plots an account's position **once usage readings exist**; before that the chart is the boundaries alone  **The viewer half is built** - per-window cards, the three boundary kinds in plain words, the band chart, drawn crossings and the time scrubber, reusing `boundaryAt`/`findCrossings` rather than restating the model. **Presets and in-place editing are built too** — a preset selector and editable per-window fields on the chart itself — and the unused `writeWindows` was removed rather than shipped as dead code. The windows stay editable from `/settings`, so this row is what a person can do *from the chart*, not whether they can change a window at all. | 86 | `done` |
...
| **48** | Plugin package — MCP config, hook config, and the command line in one install. **Consumes the published package rather than carrying a copy of the binary** | 30, 42, 89 | `done` |
| **49** | `/setup-agent-standup` — registers the scheduled task, then **proves it works** with a live call | 48 | `done` |
...
| **51** | Runs: a new run whenever the model or effort changes; the hook reports the model per call | 50 | `done` |
| **52** | Price table and cost, always recomputable from the token counts | 51 | `done` |
| **53** | Aggregation: cost per item, per session, **per stage** | 52 | `done` |
...
| **59** | The planner: sort by priority, pack against headroom, deterministic ordering | 57, 58 |`done` |
| **60** | Launch prompts composed server-side; dispatch and dispatch-claimed recorded | 59 |`done` |
| **61** | The launcher script and its scheduled task, per machine | 55, 60 |`done` |
| **62** | Failed-launch detection: dispatched, never claimed, past the threshold | 60 |`done` |
| **63** | Minting: triggered by demand, scans your sources, never mints the same thing twice | 26, 58 |`done` |
| **64** | Wait-for-crew: the held-open endpoint and the polling implementation, both bounded by the visibility horizon and returning identically, plus the crew digest. **The implementation follows the binding, never the caller.** The binary itself is #79, not here | 24, 54, 79 |`done` |
| **65** | Nudge to background a command, using how long that command has taken before | 46, 54 |`done` |
...
| PR | Delivers | Needs | Status |
|---|---|---|---|
| **66** | Scores: two of them per facet, the agent's frozen, accepting copies it to yours | 51 |`done` |
| **67** | Working out how it went on its own: review rounds, finding severity, rework, steering | 17, 53, 66 |`done` |
| **68** | Review UI on the since-your-last-visit card: sliders, only the facets in play, never blocks Seen | 38, 66 |`done` |
| **69** | Flagged runs — "we tried a cheaper model here, is this up to standard?" | 68 |`done` |
| **70** | Picker: store recommendations, discourage overrides at spawn, record when you override anyway | 67 |`done` |
| **71** | Picker: how strongly to recommend, how often to experiment, bias experiments to low-risk work | 70 |`done` |
...
| **74** | Project view and progress view  **Delivered by the rescoped M10 task for the projects rollup and grid** - the project view and its progress rollup are live at `/projects`. | 72 | `done` |
| **75** | Filters and search: area, repo, state, who's on it, priority | 36 | `done` |
| **76** | **Mobile** — P3. A different flow, not a squeezed desktop: a list with a status picker instead of drag, filters in a sheet, thumb-sized sliders, and you can still mint work  **Delivered by the rescoped M10 tasks for the triage pages and the mobile flow** - two triage pages and 44px touch targets, then a status picker for moving an item and the filters in a sheet. Minting stays reachable at phone widths. | 68, 73, 75 | `done` |
| **96** | **Retiring a reference row** — P3, optional. Surfaces the `archived_at` the schema already carries: `PATCH /repos/{id}`, `/areas/{id}`, `/people/{id}` sets it, archived rows drop out of pickers and default `GET` lists but still resolve everywhere they are already referenced. Plus the inverse — `DELETE` with an explicit hard-delete flag, allowed **only** when a reference count across every referring table is zero, refused with the counts otherwise  **Delivered by the rescoped M10 task for retiring a reference row** - archiving a repo, area or profile drops it from pickers and default lists while it still resolves where referenced, and the hard delete is refused with counts unless nothing refers to it. | 91, 92 | `done` |
| **117** | **A column is bounded and scrolls inside itself.** Backlog renders all 68 items in one unbroken column, so the page grows to the length of the longest column and the other three are dead space beside it. Give a column a max height that does not exceed the viewport, a min height so an empty column still reads as a column, and its own scroll region — the board's header and the other columns stay put while one column scrolls  **Delivered by the rescoped M10 task that bounded the board** - each column is bounded, pages, and states plainly when it is empty. | 73 | `done` |
| **118** | **Filter and sort the board from the board.** Filter by area, repo, agent, actor, profile and priority; sort by priority, name, created and last-updated, each ascending or descending; and a search box — per column or one for the whole board, whichever survives the design. **#75 is marked `done` and delivers the API filters, but nothing in the UI reaches them**, which is why this is a front-end row and not a re-open. Pairs with #118: a tag on the detail page is the same filter applied  **Delivered by the rescoped M10 task for the board's filter bar** - filter by area, repo, state, assignee and priority, with sorting and search, reaching the API filters from the board itself. | 75, 73 | `done` |
| **119** | **Tags on the detail page are links back to a filtered board.** Clicking the area, repo, priority, state or assignee chip on an item returns to the kanban with that filter already applied. The filter set is #117's; this row is only the navigation | 117, 72 | `done` |
| **120** | **The detail page renders its markdown.** Item bodies show raw `##`, `###` and pipe tables as literal text instead of rendered headings and tables — every imported item carries a full markdown brief, so this affects the majority of the corpus. Also the page's vertical budget: subtasks, artifacts and history each render fully expanded and stack to an extremely long page. Artifacts and subtasks behind a control (collapsed, or a dialog), and **history paginated** rather than every event always present  **Delivered by the rescoped M10 task for the detail page** - item bodies render their markdown, and the page is broken into tabs. | 72 | `done` |
| **131** | **Titles are for people; the technical detail belongs in the body.** From a first read of the board (2026-08-14): *"the title right now is too technical and not user facing — titles should be user facing, I should read it and know what it means, the technical details can be in the body."* Every imported title is a work order written by and for an agent — *"agent-standup #102 - route the four raw event writes through appendEvent"* — so a board scanned by a human reads as a wall of internal references. **This is not #107.** That row added `headline` as a one-line BLUF *returned by reads*; this is about what goes in `title` in the first place, which is a convention plus a UI question, not a field. Three parts: state the convention somewhere it is read at creation time; have the create surfaces (and the item form) reflect it; and decide what happens to the imported titles — leave them, or backfill a readable title and keep the original in the body. **As shipped:** the convention is one exported string carried into all four creates' `contract.rules`, so `describe_tool` serves it on demand and every `invalid_input` refusal already points there — no per-turn cost in any tool summary. It **advises rather than refuses**, because no predicate is right about every title (`Inbox` is a legitimate one-word title), so a create that departs from it succeeds and carries a `titleAdvice` note; DECISIONS.md §18 has the argument. **Stored titles are left as they are** — they are live data, `headline` already carries a readable line beside them on every read, and a bulk rewrite of the one recognisable field would exceed the problem it solves. The item form is not part of this: no create-or-edit form exists in the UI yet. **Interacts with #105:** search matches on title and body, so moving detail out of titles changes what is findable, and the two should be settled together. Filed from field feedback, 2026-08-14 | 72 | `done` |
| **132** | **The plan view is a timeline, not a dump.** On the item detail page (2026-08-14): *"a plan view should be a bit more organised — start with a summary or BLUF, click here to see the snapshot history, but maybe the latest snapshot is more prominent and easy to view and you can go back to previous snapshots, and then review artifacts."* Plan, artifacts and history all render at full length stacked down one column, so the page grows without bound and the live state is no more prominent than any superseded one. Wanted: a summary at the top, the latest snapshot rendered prominently, earlier snapshots reachable but collapsed, and review artifacts as their own section. **Get a visual reviewer on it rather than designing it in a PR description** — same instruction given for #122. Overlaps #120 (which collapses artifacts and paginates history) and #74 (the progress view); this is the plan-specific half. Filed from field feedback, 2026-08-14 | 72, 120 | `done` |
| **121** | **Subtasks are visual, and not loose on the board.** Subtasks appear on the board as their own cards, so a parent and its children compete as peers. Take them off the board and reach them from the parent — a control on the card that expands it into its children. Depends on #74 for the project/progress view it borrows from | 74, 73 | `done` |
...
> behaviour under test and confirmed no test noticed. That is exactly the gap mutation testing
> closes: not "did tests run", but "would this test actually fail if the behaviour it names
> regressed". (#95 automated this in CI; that job was removed on 2026-09-04 for cost, and the
> question is now answered by hand-mutating your own diff — see `docs/ci-required-checks.md`. The bar
> is unchanged; only who applies it moved.) A
