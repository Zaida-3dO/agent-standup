...
> **⚠️ A blank status is a claim about this file, not about the code — verify before building.**
> The rule above is only as good as the table's freshness, and the table goes stale in exactly the
> case where the work went *well*: a row gets delivered under an adjacent piece of work that never
> comes back to tick it. Four rows were found built-but-blank in a single session, and three of them
> had a crew dispatched onto them before the check caught it. **Read the source for the row you are
> about to pick up and confirm it is genuinely absent.** For anything under M10 the board is the
> source of truth over this table outright — see the rescope note on that milestone.
