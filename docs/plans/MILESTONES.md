...
| **76** | **Mobile** — P3. A different flow, not a squeezed desktop: a list with a status picker instead of drag, filters in a sheet, thumb-sized sliders, and you can still mint work  **Partly delivered by the rescoped M10 task for the triage pages** - two triage pages and 44px touch targets are built. Still open, and the reason this row stays pickable: the status picker standing in for drag, and the filters in a sheet. | 68, 73, 75 | |
