...
| **140** | **An unset capability is invisible, and the detection for it already exists.** `visual_review.doc` and `notify.doc` are both `null` on this installation, which switches off more than a convenience: the visual-review gate becomes **unsatisfiable** — two repositories are registered `needsVisualReview: true`, so every item minted against them inherits a requirement nothing can fulfil — and `notify.doc` null means escalation, including the escalation that puts a blocked item on somebody's list, reaches nobody. Neither announces itself, so a 26-PR front-end milestone merged without a single render and nothing anywhere said why. **The missing piece is a reader, not a detector, and that is what makes this cheap.** `SCHEMA.md` §17.5 already specifies the warning verbatim — *"14 items require a visual review and `visual_review.doc` is not set"* — and **#24 already ships the check that produces it**: the liveness sweep re-verifies configured capability document paths and returns `capabilityChecks`, recording `{ last_checked_by, last_checked_at, result }`. That result has had no consumer, so the condition was detected on every sweep, written down in the schema, and still went unnoticed the entire time it was true. **Deliver the surfacing half**: the §17.5 warning on `/settings` and in the sweep's own output, naming the dependent count rather than only that the setting is empty — a bare *"not set"* reads as a default, while *"14 items require this"* reads as a fault. **Scope is the warning only.** Giving the two settings values is a `put_setting`, not work, and belongs in `BEFORE-GA.md` **G2** where it is already recorded as a close condition; this row is what stops the *next* unset capability waiting to be discovered by hand. Related: **I18** in `INTERVENTIONS.md` is the per-merge version, and ships as a nudge until this row makes the gate satisfiable. Filed from the M10 retrospective, 2026-08-19 | 24, 77 | |
| **141** | **`needsVisualReview` should be a question the caller answers, not a default it inherits silently.** Settled by the owner, 2026-08-19, against the alternative of flipping this repository's row to `true`: **`false` is the correct value for most pull requests here**, so a `true` default would be the same mistake inverted — a value nobody chose, this time erring towards a gate that does not apply. The answer is not a better default but **making the field one somebody has to think about**. **The mechanism this needs already exists and is the reason the row is cheap.** `needsVisualReview` is deliberately `.optional()` rather than `.default(false)` on every creation path (#126), precisely so the handler can tell *the caller said `false`* apart from *the caller said nothing* — a `.default()` would resolve before the handler runs and lose exactly that. So the omission is already detectable at the point of creation; nothing is inferred and nothing is guessed. **Wanted: a nudge on omission, not a refusal** (owner's call, 2026-08-19). A create that names a repository and omits the field gets told it inherited a value nobody chose, naming the two answers rather than picking one. **A refusal was considered and rejected**, and the reason generalises: the field has a sane inherited value, so blocking a create over an unanswered question stops work that would have been correct anyway — the catalogue's own guidance is to prefer the weakest level that works, and a nudge is enough to make somebody think when the fallback is not wrong, only unchosen. It also keeps #126's inheritance intact and breaks no existing caller, so this needs no per-repository opt-in to be safe to ship on. **Prefer this to catching it at merge.** I18 fires at the gate, where the work is already built and the cost of the answer is a rebuild; this fires at mint, where the answer is one question with the diff still ahead of the caller. It also avoids I18's genuinely hard half — no classifier deciding which changed paths are user-facing, because a person is being asked rather than a heuristic consulted. Filed from the M10 retrospective, 2026-08-19 | 126, 92 | |
