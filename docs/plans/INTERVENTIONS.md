...
| **I18** | **A user-facing change reaching the merge gate on a repo that requires no visual review.** A `commit` artifact whose changed paths are front-end surfaces, on an item whose `needs_visual_review` is false — so the merge asks for an approving `code_review` and nothing ever renders the result. **Same shape and same phase as I10**: a condition on *this* merge, evaluated against state the server already holds, inexpressible as a command matcher. The server has both halves — the item's flag and the artifact's paths — so it can say which surfaces changed and that nothing looked at them. **The situation this is drawn from is the reason it is not simply "set the flag".** Across a 26-PR milestone an auth change left the entire front end returning 401: every PR green, every PR correct in isolation, no textual conflict, and `npm run build` passing throughout — compiling is not the same claim as loading. It was found by a person opening a page. **Two adjacent failures made it possible, and an entry that only fires on the item misses both.** First, the flag is *inherited from the repo row* (MILESTONES #126), and this product's own repo row carried `false` while both sibling UI repos carried `true` — a one-row config mistake that no diff, review or check would ever surface, which is exactly the class an intervention is for. Second, `visual_review.doc` was null the whole time, meaning the gate was **unsatisfiable even had the flag been set** — so an entry that fires without checking the capability would push items into a gate with no way through, the wedge SCHEMA.md §17.5 already warns about. **So it must report the repo row and the capability, not just the item**, and it is a nudge rather than a block until `visual_review.doc` is set: blocking on a gate nobody can satisfy stops work without protecting anything. Promote it to block-overridable once the capability is real. From the M10 retrospective, 2026-08-19 | `pre` | `orchestrator` | nudge (prominent) | immediate | |
