...
| **I12** | **A machine-wide process kill** — anything not scoped to PIDs the asking crew registered. Already has a service call in `kill_guard`, so this entry is wiring rather than new detection | `pre` | `agent` | hard block | immediate | |
