// The HTTP adapter's `POST /runs/{id}/accept` endpoint over `accept_run_score`
// (SCHEMA.md §19). A thin shell over one `service.call`.
//
// Accepts the run's agent scores as a person's own. A write, so `POST`. The
// body names who is accepting and optionally which facets; omitting the
// facets accepts every one carrying an agent score and no user score.
//
// It exists because `accept_run_score` had no route at all — it was reachable on MCP
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    const result = await service.call("accept_run_score", { ...body, runId: id }, { caller });
