// The HTTP adapter's `POST /interventions/{id}/score` endpoint over
// `score_intervention` (SCHEMA.md §19). A thin shell over one `service.call`.
//
// The `intervention_events` row being rated belongs in the path where it
// cannot be omitted; the score, who is rating and the optional note are the
// body.
//
// It exists because `score_intervention` had no route at all — it was
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    const result = await service.call("score_intervention", { ...body, eventId: id }, { caller });
