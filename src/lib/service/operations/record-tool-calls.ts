...
// readings) all read what this writes, so the shape below is load-bearing
// in a way most operations are not.
//
// ── A batch, not a single call ─────────────────────────────────────────
//
// The input is an array of calls. The reason is the shape of the client:
// the hook runs on the critical path of every tool call, and a telemetry
// write that costs a round trip per call would add latency to every action
// an agent takes — for data that nothing reads in real time. So the hook
// spools locally and flushes in batches, and this operation is the server
// half of that contract.
//
// One consequence worth stating plainly, because it is what makes the batch
// safe: **`ts` is supplied by the caller, not defaulted to now.** A batch
// flushed five minutes after the fact must record when each call happened,
// not when it was uploaded, or every cost-per-stage figure #53 computes is
...
//      this column exists"). A stage attribution that a client can assert
//      is one that a buggy client can silently corrupt, and the corruption
//      would be invisible — there is nothing to check it against.
//
...
// call. That is a real inaccuracy and it is accepted here rather than
// hidden, because the alternative — a correlated lookup into `events` per
// row, which §10 explicitly rejects as the reason the column is
// denormalised at all — costs more than the error. The error is bounded by
// the flush interval, which is short by construction, and it is zero for
// the common case of a session flushing while it still holds the item.
//
...
// reason. So a call with no resolvable assignment is *recorded*, not
// refused. Refusing it would mean the system measures only the work that
// was already tracked, which is a survivorship bias baked into the data the
// M9 picker learns from — and unminted work is exactly the work worth
// knowing about.
import { z } from "zod";
...
/**
 * The most calls accepted in one flush.
 *
 * A cap on the *batch* as well as on the fields, for a reason the field
 * caps do not cover: a single request holding an unbounded array is a
 * memory cost on the server before any cap can be applied to it, since the
 * whole body is parsed before the handler runs. 500 is comfortably more
 * than a normal flush interval produces (an agent working hard makes a few
 * calls a minute) while keeping the largest legal request bounded at a few
 * megabytes even when every field in it is at its own cap.
 *
 * Over-sized batches are **refused**, not truncated — unlike the field
 * caps. A truncated field still records the call it describes; a truncated
 * batch silently discards whole calls, and a client that cannot tell how
 * many of its rows landed cannot retry correctly. Refusing tells it
 * plainly to send fewer.
 */
 ...
 * bound turns that into an `invalid_input` naming the offending row, which
 * is the difference between a client that can fix its bug and one that sees
 * the entire flush fail forever.
 *
...
const callSchema = z
  .object({
    ...
    tool: z.string().min(1),
    /** When the call happened, per the client's clock. See the note above on why this is not `now()`. */
    ts: z.coerce.date(),
    /** The command text, for shell-shaped tools. Absent for tools that carry none. */
...
const inputSchema = z
  .object({
    /**
     * Whose calls these are. Required — §10 says `session_id` is "Always
     * present", and it is the only field that ties a ghost session's rows
     * together at all.
     */
    sessionId: z.string().min(1),
    calls: z.array(callSchema).min(1).max(MAX_BATCH_SIZE),
  })
  .strict();
...
export interface RecordToolCallsOutput {
   ...
   *
   * Returned rather than kept internal so a client can see that its
   * payloads are being clipped without reading the table back. A number
   * that climbs steadily is a hook sending file bodies where it means to
   * send commands — a bug that is otherwise completely silent, because a
   * truncated row looks exactly like a small one from the client's side.
   */
 ...
 * which the item transitions between the two reads — which would stamp a
 * batch with a state the item was never in while these calls happened.
 */
