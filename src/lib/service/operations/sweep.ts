// `sweep` — the trigger for the liveness ladder (MILESTONES.md #99,
// SCHEMA.md §2, §17.5).
//
// The ladder itself is `sweepLiveness` (src/lib/liveness.ts, row #24) and is
// complete: it walks live assignments, moves them along running → stalled →
// dead, releases what died, escalates what has exhausted its resume attempts,
// and re-checks the capability documents. What it never had was a caller. A
// sweep nothing invokes is a reclamation policy the system describes but does
// not have, and the visible symptom is a claim that can never be handed back —
...
//
// This operation is that caller, and it is deliberately a plain registered
...
//      replica holding it is the one that restarts. An operation is invoked by
//      whatever the deployment already trusts to run things on a schedule —
//      a cron entry, a scheduled task, the command line — and that is a
