...
export const SETTINGS_REGISTRY = {
  ...
  "crew.wait_timeout_seconds": define({
    ...
    help: "How long a `standup crew wait` call is held open before returning empty, and the ceiling its --timeout is clamped to. Sized to stay inside the shortest prompt-cache lifetime a session may be given, which is not always signalled — a wait that outlives the cache costs more than the wait saves.",
    category: "Crew",
    appliesWhen: "next-call",
    sensitive: false,
    irreversible: false,
  ...
  "crew.wait_poll_interval_seconds": define({
    ...
    help: "How often a `standup crew wait` re-reads the ledger while it waits. A wait polls whenever it has no signal to be woken by, which is every wait the command line and the API serve, so this interval decides both how quickly an orchestrator hears that its crew did something and how much load a waiting session costs.",
    category: "Crew",
    appliesWhen: "next-call",
    sensitive: false,
    irreversible: false,
